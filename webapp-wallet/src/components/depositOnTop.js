...
import { depositOnTop, beforeDepositOnTop } from '../actions/depositontop-actions';
...
class DepositOnTop extends Component {
  ...
  handleClick = async () => {
    ...
    const files = await beforeDepositOnTop(config, wallet, abi);
    const nodeEth = files.config.nodeEth;
    const addressSC = files.config.address;
    const operator = files.config.operator;
    const res = await depositOnTop(nodeEth, addressSC, amount, tokenId, files.wallet, password, files.abi, operator);
    console.log(res);
  }
