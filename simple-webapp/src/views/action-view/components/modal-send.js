...
class ModalSend extends Component {
    ...
    getIDs = async () => {
      ...
      const idsSender = await this.props.handleGetIds(operator, sender);
      let idsReceiver = [];
      if (this.props.activeItem !== 'send0') {
        ...
        idsReceiver = await this.props.handleGetIds(operator, receiver);
      }
      let amount;
      let fee;
      if (this.amountRef.current.value) {
        amount = web3.utils.toWei(this.amountRef.current.value, 'ether');
      } else {
        amount = 0;
      }
      if (this.feeRef.current.value) {
